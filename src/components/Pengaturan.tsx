...
const Profile: React.FC<ProfileProps> = ({ employee, allUsersData, sunnahIbadahList, activities, dailyActivitiesConfig, hospitals, onUpdateProfile, onChangePassword, cities, addToast }) => {
    ...
    const [locationSearch, setLocationSearch] = useState(employee.locationName || '');
    ...
    const filteredCities = useMemo(() => {
        if (!locationSearch || locationSearch === employee.locationName) {
            return [];
        }
        if (locationSearch.length < 3) return [];
        const searchLower = locationSearch.toLowerCase();
        return cities.filter(c => c.lokasi.toLowerCase().includes(searchLower));
    }, [locationSearch, cities, employee.locationName]);

    const handleLocationSelect = (city: City) => {
        onUpdateProfile(employee.id, { locationId: city.id, locationName: city.lokasi });
        setLocationSearch(city.lokasi);
        setIsCityDropdownOpen(false);
    };
