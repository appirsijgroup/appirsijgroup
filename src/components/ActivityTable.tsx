                ...
                <table className="min-w-full text-sm text-left text-white">
                    <thead className="bg-black/20 text-xs uppercase text-blue-200">
                        <tr>
                            ...
                            <th scope="col" className="px-4 py-3">Sisa Waktu</th>
                            <th scope="col" className="px-4 py-3 text-center">Tautan Online</th>
                            <th scope="col" className="px-4 py-3 text-center">Aksi Presensi</th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            return (
                                <tr key={item.id} className="hover:bg-white/5 border-b border-gray-800 last:border-b-0">
                                    ...
                                    <td className="px-4 py-4">
                                        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${item.isTeamSession ? 'bg-purple-500/20 text-purple-300' : 'bg-sky-500/20 text-sky-300'}`}>
                                            {item.isTeamSession ? 'Presensi Tim' : 'Kegiatan Umum'}
                                        </span>
                                    </td>
                                    <td className="px-4 py-4 font-mono">{item.startTime} - {item.endTime}</td>
                                    <td className="px-4 py-4"><span className={`font-bold ${statusClass}`}>{statusLabel}</span></td>
                                    <td className="px-4 py-4 font-mono text-base font-semibold">
                                        {countdownLabel && (
                                    ...
                                    <td className="px-4 py-4 text-center">
                                        <div className="flex items-center justify-center gap-2">
                                            {item.zoomUrl && (
                                                <a href={item.zoomUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center px-3 py-2 text-xs font-semibold rounded-lg bg-blue-600 hover:bg-blue-500 text-white transition-colors shadow" title={item.zoomUrl}>
                                                    <ZoomIcon className="w-4 h-4 mr-1.5" />
                                                    Zoom
                                                </a>
                                            )}
                                            {item.youtubeUrl && (
                                                 <a href={item.youtubeUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center px-3 py-2 text-xs font-semibold rounded-lg bg-red-600 hover:bg-red-500 text-white transition-colors shadow" title={item.youtubeUrl}>
                                    ...
                                    <td className="px-4 py-4">
                                        <div className="flex items-center justify-center space-x-2 min-w-[180px]">
                                            {(item.isTeamSession && item.attendanceMode === 'leader' && !isCreator) ? (
                                                <div className="py-2.5 px-3 rounded-lg font-semibold flex items-center justify-center bg-gray-600/50 text-white/80 shadow-md text-xs w-full">
                                                    Presensi oleh Atasan
                                                </div>
                                            ) : isSubmitted ? (
                                                <div className="w-full flex flex-col items-center">
                                                    <p className={`w-full text-center py-2.5 px-3 rounded-lg font-semibold flex items-center justify-center shadow-md text-xs ${
                                                        attendanceStatus.status === 'hadir' ? 'bg-green-500/50 text-green-200' : 'bg-red-500/50 text-red-200'
                                                    }`}>
                                                        {attendanceStatus.status === 'hadir' ? <CheckIcon className="h-4 w-4 mr-1"/> : <XIcon className="h-4 w-4 mr-1"/>}
                                                        {attendanceStatus.status === 'hadir' ? 'Hadir (Terkirim)' : 'Tidak Hadir'}
                                                    </p>
                                                </div>
                                            ) : isActionable ? (
                                                <>
                                                    <button onClick={() => onHadir(item.id)} className="w-1/2 py-2 px-3 rounded-lg font-semibold flex items-center justify-center transition-all bg-green-500 hover:bg-green-400 text-white shadow-md">
                                                        <CheckIcon className="h-5 w-5 mr-1" /> Hadir
                                                    </button>
                                                    <button onClick={() => onTidakHadir({id: item.id, name: item.name})} className="w-1/2 py-2 px-3 rounded-lg font-semibold flex items-center justify-center transition-all bg-red-500 hover:bg-red-400 text-white shadow-md">
                                                        <XIcon className="h-5 w-5 mr-1" /> Tidak
                                                    </button>
                                                </>
                                            ) : (
                                                <div className={`py-2 px-3 rounded-lg font-semibold flex items-center justify-center bg-gray-600/50 text-white/80 shadow-md text-xs w-full`}>
                                                    {statusLabel}
                                                </div>
                                            )}
                                        </div>
