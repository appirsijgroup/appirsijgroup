                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            {selectedWeekDays.map(day => (
                                                <th key={day} scope="col" className={`px-2 py-3 font-bold text-center w-20 min-w-[80px] ${isCurrentMonthView && day === todayDay ? 'bg-teal-700' : 'bg-gray-800'}`}>
                                    ...
                                    <tbody>
                                        {Object.entries(groupedActivities).map(([category, activities]) => (
                                            <Fragment key={category}>
                                                <tr className="group"><td className="px-3 py-2 font-bold text-teal-200 sticky left-0 z-10 bg-gray-700 group-hover:bg-gray-600" colSpan={selectedWeekDays.length + 2}>{category}</td></tr>
                                                {activities.map(activity => {
                                                    ...
                                                    return (
                                                        <tr key={activity.id} className="group border-b border-gray-700">
                                                            <td className="px-3 py-3 font-medium text-left sticky left-0 bg-gray-800 group-hover:bg-gray-700 z-10">{activity.title}</td>
                                                            <td className="px-3 py-3 font-semibold text-center sticky left-[250px] bg-gray-800 group-hover:bg-gray-700 z-10">
                                                                {(() => {
                                                                    const isDailyTask = activity.monthlyTarget > 7;
                                                                    if (isDailyTask) {
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            {Array.from({ length: daysInMonth }, (_, i) => i + 1).map(day => (
                                                <th key={day} scope="col" className={`px-2 py-3 font-bold text-center w-12 min-w-[48px] ${isCurrentMonthView && day === todayDay ? 'bg-teal-700' : 'bg-gray-800'}`}>{day}</th>
                                            ))}
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {Object.entries(groupedActivities).map(([category, activities]) => (
                                            <Fragment key={category}>
                                                <tr className="group"><td className="px-3 py-2 font-bold text-teal-200 sticky left-0 z-10 bg-gray-700 group-hover:bg-gray-600" colSpan={daysInMonth + 2}>{category}</td></tr>
                                                {activities.map(activity => (
                                                    <tr key={activity.id} className="group border-b border-gray-700">
                                                        <td className="px-3 py-3 font-medium text-left sticky left-0 bg-gray-800 group-hover:bg-gray-700 z-10">{activity.title}</td>
                                                        <td className="px-3 py-3 font-semibold text-center sticky left-[250px] bg-gray-800 group-hover:bg-gray-700 z-10">{activityProgressCounts[activity.id] || 0} / {activity.monthlyTarget}</td>
